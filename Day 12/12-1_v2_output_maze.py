...
for x in range(len(lines[0].replace("\n", ""))):
    for y in range(len(lines)):
        line = lines[y].replace("\n", "")
        nodes.append([line[x], [x,y], (int(len(lines) * len(line) + 1)), False, -1, [], False])
...
def printMaze():
    ...
    for y in range(len(grid[0])):
        for x in range(len(grid)):
            # print(nodes[grid[x][y][1]][6])
            isSolved = nodes[grid[x][y][1]][6]
            buffer = buffer + f"{'[' if isSolved else ' '}{grid[x][y][0]}{'' if isSolved else ''}"
        buffer = buffer + "\n"
    print(buffer)
