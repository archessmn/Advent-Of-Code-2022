...
for x in range(len(lines[0].replace("\n", ""))):
    for y in range(len(lines)):
        ...
        if line[x] == "a":
            # nodes[-1][2] = 0
            startPointers.append(len(nodes) - 1)
        if line[x] == "E":
            ...
            for i in range(-1, 2):
                ...
                if 0 <= iCoord <= len(grid[0]) - 1:
                    if abs(o) != abs(i):
                        ...
                        if node[0] == "a" or inspectingGrid[0] == "E":
                            if (node[0] == "a" and inspectingGrid[0] == "b")  or (node[0] == "z" and inspectingGrid[0] == "E"):
...
shortest = (int(len(lines) * len(line) + 1))

for s in range(len(startPointers)):

    cycleNodes = copy.deepcopy(nodes)

    singleStartNode = cycleNodes[startPointers[s]]

    cycleNodes[startPointers[s]][2] = 0
    ...
    for n in range(len(nodes)):
        ...
        for i in range(len(nodes)):
            if (cycleNodes[i][2] < shortestPathToStart) and (not cycleNodes[i][3]):
                shortestPathToStart = cycleNodes[i][2]
                shortestPointer = i
        
        for i in range(len(cycleNodes[shortestPointer][5])):
            inspectedChildNode = cycleNodes[cycleNodes[shortestPointer][5][i]]
            if not inspectedChildNode[3]:
                if cycleNodes[shortestPointer][2] + 1 < inspectedChildNode[2]:
                    cycleNodes[cycleNodes[shortestPointer][5][i]][2] = cycleNodes[shortestPointer][2] + 1
                    cycleNodes[cycleNodes[shortestPointer][5][i]][4] = shortestPointer
        
        cycleNodes[shortestPointer][3] = True

# print(nodes[3177])
    for y in range(len(grid[0])):
        for x in range(len(grid)):
            isSolved = True if cycleNodes[grid[x][y][1]][2] != (int(len(lines) * len(line) + 1)) else False
            print(f"{'[' if isSolved else ' '}{grid[x][y][0]}{'' if isSolved else ''}", end="")
        print("")

    if cycleNodes[endPointer][2] < shortest:
        shortest = cycleNodes[endPointer][2]

    
    print(shortest)

    print(cycleNodes[endPointer][2])
